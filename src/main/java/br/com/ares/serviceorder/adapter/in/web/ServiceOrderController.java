...
public class ServiceOrderController {
    ...
    ServiceOrder create(@Valid @RequestBody CreateOrderRequest r) {
        return orders.create(new ServiceOrderUseCase.CreateOrderCommand(
                r.customerId(), r.assetId(), toCommands(r.lines()), r.title(), r.description(), r.priority(),
                r.assignedTechnicianId(), r.dueAt(), r.scheduledStartAt(), r.scheduledEndAt()));
    }
    ...
    @PreAuthorize("hasAuthority('SERVICE_ORDER_READ') and !hasRole('CUSTOMER')")
    List<ServiceOrderUseCase.TechnicianView> technicians() {
        return orders.listTechnicians();
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasAuthority('SERVICE_ORDER_READ')")
    ServiceOrder get(@PathVariable UUID id) {
    ...
    @PreAuthorize("hasAuthority('SERVICE_ORDER_UPDATE') and !hasRole('CUSTOMER')")
    ServiceOrder status(@PathVariable UUID id, @Valid @RequestBody ChangeStatusRequest r) {
        return orders.changeStatus(id,
    ...
    @PutMapping("/{id}/quote")
    @PreAuthorize("hasAuthority('SERVICE_ORDER_UPDATE') and !hasRole('CUSTOMER')")
    ServiceOrder quote(@PathVariable UUID id, @Valid @RequestBody UpdateQuoteRequest r) {
        return orders.updateQuote(id,
                new ServiceOrderUseCase.UpdateQuoteCommand(r.assetId(), toCommands(r.lines())));
    }

    @PatchMapping("/{id}/planning")
    @PreAuthorize("hasAuthority('SERVICE_ORDER_UPDATE') and !hasRole('CUSTOMER')")
    ServiceOrder planning(@PathVariable UUID id, @Valid @RequestBody UpdatePlanningRequest r) {
        return orders.updatePlanning(id, new ServiceOrderUseCase.UpdatePlanningCommand(r.assignedTechnicianId(),
                r.dueAt(), r.scheduledStartAt(), r.scheduledEndAt()));
    }

    @GetMapping("/{id}/timeline")
    @PreAuthorize("hasAuthority('SERVICE_ORDER_READ')")
    List<ServiceOrderUseCase.TimelineEvent> timeline(@PathVariable UUID id) {
        return orders.timeline(id);
    }

    private List<ServiceOrderUseCase.QuoteLineCommand> toCommands(List<QuoteLineRequest> lines) {
        return lines.stream()
    ...
    record UpdatePlanningRequest(UUID assignedTechnicianId, Instant dueAt, Instant scheduledStartAt,
