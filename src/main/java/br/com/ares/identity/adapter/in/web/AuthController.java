...
import br.com.ares.identity.adapter.in.web.request.ForgotPasswordRequest;
import br.com.ares.identity.adapter.in.web.request.LoginRequest;
import br.com.ares.identity.adapter.in.web.request.TokenRequest;
...
public class AuthController {
    ...
    @PostMapping("/login")
    public AuthUseCase.AuthenticationResult login(@Valid @RequestBody LoginRequest body, HttpServletRequest request) {
        return auth.authenticate(new AuthUseCase.LoginCommand(body.email(), body.password(), request.getRemoteAddr()));
    }

    @PostMapping("/refresh")
    public AuthUseCase.AuthenticationResult refresh(@Valid @RequestBody TokenRequest body) {
    ...
    public ResponseEntity<Map<String, String>> forgotPassword(@Valid @RequestBody ForgotPasswordRequest body) {
    ...
    @GetMapping("/me")
    public AuthUseCase.MeResult me() {
        return auth.me();
    }
