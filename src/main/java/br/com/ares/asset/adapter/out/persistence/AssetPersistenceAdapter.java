...
class AssetPersistenceAdapter implements AssetRepository {
    ...
    @Override public Asset save(Asset value) { return toDomain(repository.save(toEntity(value))); }
    @Override public Optional<Asset> findByIdAndTenantId(UUID id, UUID tenantId) {
        return repository.findByIdAndTenantId(id, tenantId).map(this::toDomain);
    }
    @Override public List<Asset> findAllByTenantId(UUID tenantId) {
        return repository.findAllByTenantIdOrderByNameAsc(tenantId).stream().map(this::toDomain).toList();
    }
    @Override public List<Asset> findAllByTenantIdAndCustomerId(UUID tenantId, UUID customerId) {
        return repository.findAllByTenantIdAndCustomerIdOrderByNameAsc(tenantId, customerId).stream()
                .map(this::toDomain).toList();
    }
    @Override public boolean existsByIdAndTenantIdAndCustomerId(UUID id, UUID tenantId, UUID customerId) {
        return repository.existsByIdAndTenantIdAndCustomerId(id, tenantId, customerId);
    }
    private AssetJpaEntity toEntity(Asset v) {
        var e = new AssetJpaEntity(); e.id=v.id(); e.tenantId=v.tenantId(); e.customerId=v.customerId();
        e.type=v.type(); e.name=v.name(); e.brand=v.brand(); e.model=v.model(); e.serialNumber=v.serialNumber();
        try { e.attributesJson=objectMapper.writeValueAsString(v.attributes()); }
        catch (Exception ex) { throw new IllegalArgumentException("Invalid asset attributes", ex); }
    ...
    private Asset toDomain(AssetJpaEntity e) {
        ...
        } catch (Exception ex) { throw new IllegalStateException("Invalid stored asset attributes", ex); }
    }
}
